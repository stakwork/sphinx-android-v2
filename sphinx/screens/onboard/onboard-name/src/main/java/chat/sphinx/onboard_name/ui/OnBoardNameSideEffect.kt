...
internal sealed class OnBoardNameSideEffect: SideEffect<Context>() {
    object GetContactsFailed: OnBoardNameSideEffect() {
        override suspend fun execute(value: Context) {
            SphinxToastUtils().show(value, R_onboard_resources.string.side_effect_get_contacts_failed)
        }
    }

    object UpdateOwnerFailed: OnBoardNameSideEffect() {
        override suspend fun execute(value: Context) {
            SphinxToastUtils().show(value, R_onboard_resources.string.side_effect_update_owner_failed)
        }
    }
}
